...
function ComposeModal({
  ...
  const initialBody = templateToUse?.body
    ...
    : isReply
      ...
      : `Dzień dobry,\n\n\n\nPozdrawiam,\nKarl Handke\nHandke Holding OÜ${createSignatureText(defaultSignature)}`;
  ...
  function applySignature(signatureId) {
    const signature = mailSignatures.find((item) => item.id === signatureId);

    setForm((current) => ({
      ...current,
      signature: signatureId,
      body: current.addSignature
        ? `${current.body.replace(/\n\n—[\s\S]*$/g, "")}${createSignatureText(signature)}`
        : current.body,
    }));
  }
      ...
      <div className="mail-signature-layout">
        ...
        <div className="mail-signature-preview-card">
          ...
          <p className="mail-signature-note">
            Logo jest pobierane z <strong>src/assets/logo.png</strong>. W finalnej
            wersji backend może osadzać je jako obraz inline albo używać publicznego URL.
          </p>
        </div>
      </div>
    </section>
...
export default function MailsPage() {
  ...
  async function handleSendDraft(message) {
    ...
    try {
      ...
      const data = await createMail(emailToPayload(newEmail));
      const createdEmail = normalizeEmail(data.mail);

      setEmails((current) => [createdEmail, ...current]);
      setPageSuccess(isSend ? "Wiadomość została zapisana jako wysłana." : "Wiadomość robocza została zapisana.");
    } catch (error) {
      setPageError(error.message || "Nie udało się zapisać wiadomości.");
      throw error;
    } finally {
      setIsSaving(false);
    }
    ...
    <section className="mails-page">
      ...
      <div className="mails-hero">
        ...
        <div className="mails-hero-actions">
          <button className="small-outline-button" type="button" onClick={loadEmails}>
            <MailOpen size={15} />
            Synchronizuj
          </button>
